...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        $.fn.date.defaultOptions = {
            ...
            theme:"date",                    //控件样式（date：日期，yearMonth：年月，year：年，datetime：日期+时间）
            mode:null,                       //操作模式（滑动模式）
            event:"click",                   //打开日期插件默认方式为点击后后弹出日期 
			splitstr:"-",					//日期分隔符 2015-05-06
            show:true
        }
        //用户选项覆盖插件默认选项   
        ...
        if(opts.theme === "datetime"){
			datetime = true;
		}else if(opts.theme === "yearMonth"){
			yearMonth = true;
		}else if(opts.theme === "year"){
			year = true;
	...
	function resetIndex(){
        ...
        function resetInitDete(){
            ...
            var nowd = new Date();
            var yyyy=parseInt(nowd.getFullYear()+"");
            initY =yyyy - opts.beginyear;
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {	
                var datestr = $("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1)+opts.splitstr+
                          $("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1)+opts.splitstr+
						$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
				
				if(year){
					datestr = $("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1);
				}else if(yearMonth){
					datestr = $("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1)+opts.splitstr+
